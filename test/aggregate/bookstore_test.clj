...
(def ddl
  ["DROP TABLE IF EXISTS book, company, author, author_books"
   ...
   "CREATE TABLE author (id integer generated by default as identity primary key, author_name VARCHAR(80), company_id INTEGER NOT NULL REFERENCES company (id));"
...
(deftest bookstore-tests
  (execute! @db-con ddl)
  (let [book-id    (-> (agg/save! model db-spec :book {:book_name "The Great Gatsby" :authors []}) :id)
        company-id (-> (agg/save! model db-spec :company {:company_name "Fishbulb"}) :id)
        book       {:book_name "Moby Dick"
                    :authors [{:author_name "Herman Melville" :company {:id company-id}}]
                    ;;:authors   [{:author_name "Herman Melville" :company_id company-id}]
                    :id        book-id}
